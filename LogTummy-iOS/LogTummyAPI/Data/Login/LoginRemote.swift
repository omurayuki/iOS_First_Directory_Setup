...
final class LoginRemote: LoginRemoteProtocol {
    ...
    func oAuthLogin(presentingForm: UIViewController?) -> Single<TWUserEntity> {
        return Single.create { observer -> Disposable in
            let swifter = Swifter(consumerKey: SwifterGeneral.TWITTER_CONSUMER_KEY.description, consumerSecret: SwifterGeneral.TWITTER_CONSUMER_SECRET.description)
            swifter.authorize(withCallback: URL(string: SwifterGeneral.CALLBACK_URL.description)!, presentingFrom: presentingForm, success: { accessToken, response in
